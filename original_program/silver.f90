...
MODULE variables
      USE nrtype
      INTEGER(I4B)                        :: numbeam=10, numvtx=11, counter_GaussSeidel
      INTEGER(I4B), PARAMETER             :: ndim=500, ndim2=6000 !ndim2はnumbeam,numvtxに関連する
	  ...
	  REAL(DP), DIMENSION(ndim)     ::  d_in1, d_out1, A1, Al1, Aw1
      REAL(DP), DIMENSION(ndim)     ::  d_l1, d_l2,rho_inner,rho_outer,rhoC	  
      ...
      REAL(DP)                                      :: E1=5.5d9, length1=100.d0, Cd=1.d0, tolerance1=1.d-10
      ...
      REAL(DP)                                      :: down, rmd1
!======       Sinker      ============================================================================
      REAL(DP)                            :: mRs=12.2d0, volRs=3.50d-3, T1x, T1y, T1z, Cds=-1.d0
...
!==========velocity===================     
     ...
     do i=1,10000001
        ...
        PPAP=dsqrt((Lcbl_xyz(1,0,1)-Lcbl_xyz(1,0,numvtx))**2+(Lcbl_xyz(2,0,1)-Lcbl_xyz(2,0,numvtx))**2)
        write(*,*) "D=", PPAP
...
!                theta1(i)=deg(1)*PI/180.d0
          ...
          do n=nmin,nmax-1
             if (y_n(n)>=Lcbl_xyz(3,0,i) .and. Lcbl_xyz(3,0,i)>=y_n(n+1)) then
                theta1(i)=0.d0/180.d0*PI
!                theta1(i)=(((y_n(n+1)-y_n(n))/(y_n(n+1)-y_n(n)))*(Lcbl_xyz(3,0,i)-y_n(n))+y_n(n))*PI/180.d0
                Flow1dt(1,0,i)=(((x_n(n+1)-x_n(n))/(y_n(n+1)-y_n(n)))*(Lcbl_xyz(3,0,i)-y_n(n))+x_n(n))*cos(theta1(i))
                Flow1dt(2,0,i)=(((x_n(n+1)-x_n(n))/(y_n(n+1)-y_n(n)))*(Lcbl_xyz(3,0,i)-y_n(n))+x_n(n))*sin(theta1(i))
                Flow1dt(3,0,i)=0.d0
             end if
          end do
        end if
...
SUBROUTINE InitialSettingsSinker
      ...
      Xs(1,0)=Lcbl_xyz(1,0,numvtx)
      Xs(2,0)=Lcbl_xyz(2,0,numvtx)
      Xs(3,0)=Lcbl_xyz(3,0,numvtx)-0.2675d0

      Xs(1,1)=Lcbl_xyz(1,0,numvtx)+0.250d0
      Xs(2,1)=Lcbl_xyz(2,0,numvtx)+0.250d0
      Xs(3,1)=Lcbl_xyz(3,0,numvtx)
      
      Xs(1,2)=Lcbl_xyz(1,0,numvtx)-0.25d0
      Xs(2,2)=Lcbl_xyz(2,0,numvtx)+0.25d0
      Xs(3,2)=Lcbl_xyz(3,0,numvtx)
      
      Xs(1,3)=Lcbl_xyz(1,0,numvtx)-0.250d0
      Xs(2,3)=Lcbl_xyz(2,0,numvtx)+0.250d0
      Xs(3,3)=Lcbl_xyz(3,0,numvtx)-0.535d0
      
      Xs(1,4)=Lcbl_xyz(1,0,numvtx)+0.250d0
      Xs(2,4)=Lcbl_xyz(2,0,numvtx)+0.250d0
      Xs(3,4)=Lcbl_xyz(3,0,numvtx)-0.535d0
      
      Xs(1,5)=Lcbl_xyz(1,0,numvtx)+0.250d0
      Xs(2,5)=Lcbl_xyz(2,0,numvtx)-0.250d0
      Xs(3,5)=Lcbl_xyz(3,0,numvtx)
      
      Xs(1,6)=Lcbl_xyz(1,0,numvtx)-0.250d0
      Xs(2,6)=Lcbl_xyz(2,0,numvtx)-0.250d0
      Xs(3,6)=Lcbl_xyz(3,0,numvtx)
      
      Xs(1,7)=Lcbl_xyz(1,0,numvtx)-0.250d0
      Xs(2,7)=Lcbl_xyz(2,0,numvtx)-0.250d0
      Xs(3,7)=Lcbl_xyz(3,0,numvtx)-0.535d0
      
      Xs(1,8)=Lcbl_xyz(1,0,numvtx)+0.250d0
      Xs(2,8)=Lcbl_xyz(2,0,numvtx)-0.250d0
      Xs(3,8)=Lcbl_xyz(3,0,numvtx)-0.535d0
      
      Xs(1,9)=Lcbl_xyz(1,0,numvtx)
      Xs(2,9)=Lcbl_xyz(2,0,numvtx)
      Xs(3,9)=Lcbl_xyz(3,0,numvtx)
...
SUBROUTINE forthSinker
      ...
      CdxS=0.5d0*rhow*Sx*Cds*(Xsdt(1,0)-Flow1dt(1,0,numvtx))*abs(Xsdt(1,0)-Flow1dt(1,0,numvtx))
      CdyS=0.5d0*rhow*Sy*Cds*(Xsdt(2,0)-Flow1dt(2,0,numvtx))*abs(Xsdt(2,0)-Flow1dt(2,0,numvtx))
      CdzS=0.d0!0.5d0*rhow*Sz*Cd*(X1dt(3,0,numvtx)-Flow1dt(3,0,numvtx))*abs(X1dt(3,0,numvtx)-Flow1dt(3,0,numvtx))
...
SUBROUTINE Forceofcurrent1
      ...
      INTEGER(I4B)                        :: i, j, k, ii, jj, n
         ...
         if ((X1dt1(2,0,i)-Flow1dt(2,0,i))<0.d0) then
          ...
          dx1(i)=Lcbl_xyz(1,0,i+1)-Lcbl_xyz(1,0,i)
          dy1(i)=Lcbl_xyz(2,0,i+1)-Lcbl_xyz(2,0,i)
          dz1(i)=Lcbl_xyz(3,0,i+1)-Lcbl_xyz(3,0,i)
...
SUBROUTINE rungeKuttaSinker
 ...
 !        rungeKob(1,0,1)=dt*(mRs*Xsdt(3,1)*Xsdt(2,0)-mRs*Xsdt(2,1)*Xsdt(3,0)+FXs(1,0))/mRs についてはsubroutine化
 !　　　　　　　call rungeK 
...
!        rungeKob(1,0,3)=(Xsdt(3,1)*Xsdt(2,0)-Xsdt(2,1)*Xsdt(3,0)+FXs(1,0)/mRs)*dt
        ...
        do j=0,0
          do i=1,3     
            ...
            Xs(i,j)=Xs(i,j) + (rungeKab(i,j,1)+2.d0*rungeKab(i,j,2)+2.d0*rungeKab(i,j,3)+rungeKab(i,j,4))/6.d0
          end do
        end do
        do i=1,3
...
SUBROUTINE ANCFSinker
      ...
      do while(emax1>tolerance1)
       ...
       write(*,*)'counter_GaussSeidel=', counter_GaussSeidel  
...
!		rho1(i)=1030.d0
    ...
    do i=1,numvtx
        Lcbl_xyz(1,0,i)=0.d0
        Lcbl_xyz(2,0,i)=0.d0
        Lcbl_xyz(3,0,i)=0.d0-real(i-1)*l_elm1(i-1)
        Lcbl_xyz(1,1,i)=0.d0
        Lcbl_xyz(2,1,i)=0.d0
        Lcbl_xyz(3,1,i)=-1.d0
        X10(1,0,i)=0.d0
...
!		end if
      ...
      X11(1,0,1)=Lcbl_xyz(1,0,1)
      X11(2,0,1)=Lcbl_xyz(2,0,1)
      X11(3,0,1)=Lcbl_xyz(3,0,1)  
      X00(1,0,1)=X10(1,0,1)
      X00(2,0,1)=X10(2,0,1)
      X00(3,0,1)=X10(3,0,1)  
      ...
      do i=2,numvtx-1
        ...
        X11(1,0,i)=S1*Lcbl_xyz(1,0,i)+S2*Lcbl_xyz(1,1,i)+S3*Lcbl_xyz(1,0,i+1)+S4*Lcbl_xyz(1,1,i+1)
        X11(2,0,i)=S1*Lcbl_xyz(2,0,i)+S2*Lcbl_xyz(2,1,i)+S3*Lcbl_xyz(2,0,i+1)+S4*Lcbl_xyz(2,1,i+1)
        X11(3,0,i)=S1*Lcbl_xyz(3,0,i)+S2*Lcbl_xyz(3,1,i)+S3*Lcbl_xyz(3,0,i+1)+S4*Lcbl_xyz(3,1,i+1) 
      ...
      X11(1,0,numvtx)=Xs(1,9)
      X11(2,0,numvtx)=Xs(2,9)
      X11(3,0,numvtx)=Xs(3,9)   
...
SUBROUTINE MassMat_TotStructure1
      ...
      do i=1,6
        ...
        do j=1,12
          jj=6*(numbeam-1)+j
          M_st1(ii,jj)=Mass1(i+6,j,numbeam)
        end do
      end do
...
SUBROUTINE KlMat1(k)
      ...
      REAL(DP)                            :: epsl_d, d
        ...
        d=dsqrt((Lcbl_xyz(1,0,k+1)-Lcbl_xyz(1,0,k))**2+(Lcbl_xyz(2,0,k+1)-Lcbl_xyz(2,0,k))**2+(Lcbl_xyz(3,0,k+1)-Lcbl_xyz(3,0,k))**2)
        epsl_d=(d-l_elm1(k))/d
...
SUBROUTINE KMat_TotStructure1
      ...
      do i=1,6
        ...
        do j=1,12
          jj=6*(numbeam-1)+j
          K_st1(ii,jj)=Kall1(i+6,j,numbeam)
        end do
      end do
...
SUBROUTINE Cal_velaccforRes1
     ...
     if(t==dt) then 
       do i=1,1
        do j=1,3
          do k=0,1
             X1dt1(j,k,i)=(Lcbl_xyz(j,k,i)-X10(j,k,i))/dt
             X1dt21(j,k,i)=0.d0
           end do
         end do
       end do
      ...
      do i=1,1
        do j=1,2
          do k=0,1
            X1dt21(j,k,i)=(4.d0*Lcbl_xyz(j,k,i)-4.d0*X10(j,k,i)-4.d0*X1dt0(j,k,i)*dt)/(dt**2)-X1dt20(j,k,i)
      ...
      X1dt21(1,0,numvtx)=(cable1dt(1)-X1dt0(1,0,numvtx))/dt
      X1dt21(2,0,numvtx)=(cable1dt(2)-X1dt0(2,0,numvtx))/dt
      X1dt21(3,0,numvtx)=(cable1dt(3)-X1dt0(3,0,numvtx))/dt
...
SUBROUTINE Update_velacc1
     ...
     do i=2,numvtx-1
      ...
      X1dt(1,0,numvtx)=cable1dt(1)
      X1dt(2,0,numvtx)=cable1dt(2)
      X1dt(3,0,numvtx)=cable1dt(3)
      X1dt2(1,0,numvtx)=(cable1dt(1)-X1dt0(1,0,numvtx))/dt
      X1dt2(2,0,numvtx)=(cable1dt(2)-X1dt0(2,0,numvtx))/dt
      X1dt2(3,0,numvtx)=(cable1dt(3)-X1dt0(3,0,numvtx))/dt
       ...
       sum=0.d0
        do i=1,2
          do j=0,1
            do k=1,3
              jj=6*(i-1)+3*j+k
              sum=sum+Mass1(ii,jj,numbeam)*X1dt2(k,j,numvtx-2+i)
            end do
          end do
        end do
        Me1(ii)=sum
       ...
       sum=0.d0
        do i=1,2
          do j=0,1
            do k=1,3
              jj=6*(i-1)+3*j+k
              sum=sum+Kl1(ii,jj,numbeam)*X1(k,j,numvtx-2+i)
            end do
          end do
        end do
        Qkl1(ii)=sum
       ...
       sum=0.d0
        do i=1,2
          do j=0,1
            do k=1,3
              jj=6*(i-1)+3*j+k
              sum=sum+Kb1(ii,jj,numbeam)*X1(k,j,numvtx-2+i)
            end do
          end do
        end do
        Qkb1(ii)=sum
       ...
       sum=0.d0
        do i=1,2
          do j=0,1
            do k=1,3
              jj=6*(i-1)+3*j+k
              sum=sum+Mass1(ii,jj,numbeam)*X1dt2(k,j,numvtx-2+i)+Kall1(ii,jj,numbeam)*X1(k,j,numvtx-2+i)
            end do
          end do
        end do
        Fext1(ii)=sum
...
!==============================================      
!      write(*,*) Fext1(7),Fout1(6*numbeam+1)
!      write(*,*) Fext1(8),Fout1(6*numbeam+2)      
!      write(*,*) Fext1(9),Fout1(6*numbeam+3)
!      pause      
...
SUBROUTINE OutputCable1
     ...
     write(141,'(3(f16.8,a1),f16.8)') t, ',', X1dt(1,0,numvtx), ',', X1dt(2,0,numvtx), ',', X1dt(3,0,numvtx)

     D=dsqrt((X1(1,0,1)-X1(1,0,numvtx))**2+(X1(2,0,1)-X1(2,0,numvtx))**2)

!-------------------------tansindo-------------------------------------------------------------------------
             write (31,'(7(f16.8,a1),f16.8)')  t, ',',   X1(1,0,1), ',',   X1(2,0,1), ',',   X1(3,0,1), ',',   X1(1,0,numvtx), ',',   X1(2,0,numvtx), ',',   X1(3,0,numvtx), ',', D
!-------------------------------------------------------------------------------------------------------------
